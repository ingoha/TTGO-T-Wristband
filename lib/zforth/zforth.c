...
static void do_prim(zf_prim op, const char *input)
{
	...
	switch(op) {
        ...
        */
        ...
        case PRIM_GET:
            {
                uint32_t *ptr = (uint32_t *) dstack[ (dsp-1) ];
                dstack[ (dsp-1) ] = *ptr;
            }
            break;
        case PRIM_SET:  // n addr --
            {
                uint32_t *ptr = (uint32_t *) dstack[ --dsp ];
                uint32_t data = dstack[ --dsp];

                *ptr = data;
            }
            break;
        case PRIM_CSET:  // n addr --
            {
                int8_t *ptr = (uint8_t *) dstack[ --dsp ];
                int8_t data = (int8_t) dstack[ --dsp] & 0xff;
        ...
        case PRIM_CGET:
            {
                uint8_t *ptr = (uint8_t *) dstack[ (dsp-1) ];
                dstack[ (dsp-1) ] = (*ptr) & 0xff;
            }
            break;
        case PRIM_CMOVE: 
            {
